        ...
        <form action={setSurge} className="flex items-end gap-3">
          <label className="text-sm text-black/55">
            ...
            <input name="surge" type="number" step="0.1" min="1" defaultValue={surge}
              className="mt-1 block w-28 rounded-lg border border-black/10 bg-white p-2 focus:border-black" />
          </label>
          <button className="rounded-lg bg-black px-4 py-2 text-sm font-semibold text-white">Save</button>
          <span className="text-xs text-black/45">Applies to new fare quotes.</span>
        </form>
      </section>

      {/* Fare config */}
      <section className="mb-8">
        <h2 className="mb-3 font-bold">Fare config (USD)</h2>
        <div className="space-y-3">
          {(fares ?? []).map((f: any) => (
            <form key={f.class} action={updateFareConfig}
              className="flex flex-wrap items-end gap-3 rounded-2xl border border-black/10 bg-[#f6f6f6] p-4">
              <input type="hidden" name="class" value={f.class} />
              <span className="w-16 font-bold capitalize">{f.class}</span>
              {(['base_fare', 'per_km', 'per_min', 'minimum_fare', 'cancellation_fee'] as const).map((field) => (
                <label key={field} className="text-xs text-black/55">
                  {field.replace('_', ' ')}
                  <input name={field} type="number" step="0.01" defaultValue={f[field]}
                    className="mt-1 block w-24 rounded-lg border border-black/10 bg-white p-2 focus:border-black" />
                </label>
              ))}
              <button className="rounded-lg bg-black px-4 py-2 text-sm font-semibold text-white">Save</button>
            </form>
            ...
            <tbody>
              {all.map((t: any) => (
                <tr key={t.id} className="border-t border-black/10">
                  ...
                  <td className="p-3">
                    <span className={ACTIVE.includes(t.status) ? 'text-[#b26b00]' : t.status === 'completed' ? 'text-brand' : 'text-black/55'}>
                      {t.status.replace('_', ' ')}
                    </span>
                  </td>
                  <td className="p-3">${Number(t.final_fare ?? t.est_fare ?? 0).toFixed(2)}</td>
                  <td className="p-3 text-black/55" style={{ maxWidth: 360 }}>
